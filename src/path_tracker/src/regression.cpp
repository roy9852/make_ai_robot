...
// void SmoothingSplineRegressor::_SetKnots(const Eigen::VectorXd& xs){
//     std::vector<double> vec;
//     vec.resize(xs.size());
//     Eigen::VectorXd::Map(&vec[0], xs.size()) = xs;
//     std::sort(vec.begin(), vec.end());
//     vec.erase(std::unique(vec.begin(), vec.end()), vec.end());
//     _knots.resize(vec.size());
//     std::copy(vec.begin(), vec.end(), _knots.begin());
// }
...
bool SmoothingSplineRegressor::CheckDOF(double size){
    ...
    if (std::isnan(_dof) || std::isinf(_dof)) _dof = 1.0;
