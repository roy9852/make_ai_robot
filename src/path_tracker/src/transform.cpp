...
geometry_msgs::msg::Point Transform::ChangePointCoordinate(double x, double y, double z, Eigen::Matrix4d rot) {
    Eigen::Vector4d p;
    p << x, y, z, 1;
    p = rot * p;
...
geometry_msgs::msg::Point Transform::ChangePointCoordinate(double x, double y, double z, Eigen::Matrix3d rot) {
    Eigen::Vector3d p;
    p << x, y, z;
    p = rot * p;
...
std::vector<geometry_msgs::msg::Point> Transform::TransformWithPosQuaternion(std::vector<geometry_msgs::msg::Point> pts, geometry_msgs::msg::Point p, geometry_msgs::msg::Quaternion q) {
    
    auto rot = Transform::GetRotationFromQuaternion(q);
    rot.col(3) << p.x, p.y, p.z, 1;
    
    std::vector<geometry_msgs::msg::Point> changedPts;
    std::transform(pts.begin(), pts.end(), std::back_inserter(changedPts), [this, &rot](const geometry_msgs::msg::Point& pt) {
            return this->ChangePointCoordinate(pt.x, pt.y, pt.z, rot);
    });
    return changedPts;
}

std::vector<geometry_msgs::msg::Point> Transform::TransformWithPosEuler(std::vector<geometry_msgs::msg::Point> pts, geometry_msgs::msg::Point p, EulerAngle e) {
...
std::vector<geometry_msgs::msg::Point> Transform::RotateWithQuaternion(std::vector<geometry_msgs::msg::Point> pts, geometry_msgs::msg::Quaternion q) {
...
std::vector<geometry_msgs::msg::Point> Transform::RotateWithEuler(std::vector<geometry_msgs::msg::Point> pts, EulerAngle e) {
...
EulerAngle Transform::GetEulerFromQuaternion(geometry_msgs::msg::Quaternion q) {
    EulerAngle e;
    ...
    e.pitch = 2.0 * std::atan2(sinp, cosp) - M_PI / 2.0;
...
geometry_msgs::msg::Quaternion Transform::GetQuaternionFromEuler(EulerAngle e) {
...
EulerAngle Transform::GetEulerFromRotation(Eigen::Matrix3d rot) {
    ...
    return Transform::GetEulerFromQuaternion(q);
}

geometry_msgs::msg::Quaternion Transform::GetQuaternionFromRotation(Eigen::Matrix3d rot) {
    double m00 = rot(0,0);
    double m01 = rot(0,1);
    double m02 = rot(0,2);
    double m10 = rot(1,0);
...
Eigen::Matrix4d Transform::GetRotationFromQuaternion(geometry_msgs::msg::Quaternion q) {
    Eigen::Matrix4d Rq;
...
geometry_msgs::msg::Quaternion Transform::MultiplyQuaternion(
...
State Transform::GetState(geometry_msgs::msg::Point p, geometry_msgs::msg::Quaternion q, geometry_msgs::msg::Twist t) {
    
    auto e = Transform::GetEulerFromQuaternion(q);
    auto t_new = t;
    t_new.linear.z = 0;
    
    double dot_product = t_new.linear.x * std::cos(e.yaw) + t_new.linear.y * std::sin(e.yaw);
    double vel_direction = (dot_product >= 0) ? 1.0 : -1.0;

    double v = vel_direction * GetNorm(t_new.linear);
