...
double State::GetDistance(const State& state) const {
    return std::sqrt(std::pow(x - state.x, 2) + 
                    std::pow(y - state.y, 2) + 
                    std::pow(z - state.z, 2));
}

double State::GetDistance(const geometry_msgs::msg::Point& pt) const {
    return std::sqrt(std::pow(x - pt.x, 2) + 
                    std::pow(y - pt.y, 2) + 
                    std::pow(z - pt.z, 2));
}

double State::GetDistance(const PathStamp& stamp) const {
    return std::sqrt(std::pow(x - stamp.x, 2) + 
                    std::pow(y - stamp.y, 2) + 
                    std::pow(z - stamp.z, 2));
}

double State::GetDistance(const double& p, const double& q, const double& r) const {
    return std::sqrt(std::pow(x - p, 2) + 
                    std::pow(y - q, 2) + 
                    std::pow(z - r, 2));
}
...
geometry_msgs::msg::Point State::toGeoPoint() {
...
geometry_msgs::msg::Point State::toGeoPoint() const {
    geometry_msgs::msg::Point pt;
    pt.x = x;
    pt.y = y;
    pt.z = z;
...
geometry_msgs::msg::Point State::toGlobalPoint(const double robot_yaw, const geometry_msgs::msg::Point& robot_pos) {
    Eigen::Matrix3d rot;
