  ...
  const rclcpp_lifecycle::State & previous_state)
{
  ...
  // Initialize lastCmd and lastState
  ...
  lastCmd.tau = 0;
  lastState.tau_est = 0;
  lastCmd.mode = PMSM;  // Set to servo mode
  lastCmd.kp = 0.0;     // Zero gains initially for safety
  lastCmd.kd = 0.0;
  
  // Initialize servoCmd to safe defaults
  servoCmd.mode = PMSM;
  servoCmd.pos = init_pose;
  servoCmd.posStiffness = 0.0;  // Zero stiffness = no torque
  servoCmd.vel = 0.0;
  servoCmd.velStiffness = 0.0;
  servoCmd.torque = 0.0;
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  // // no command received yet. Send zero effort for safety
  if (!joint_commands)
  {
    command_interfaces_[0].set_value(0.0);  // Send zero effort
    publishState();
  ...
  // Compute velocity using simple finite difference (bypass binary library)
  double dt = period.seconds();
  if (dt > 0.0 && dt < 1.0) {  // Sanity check on dt
    currentVel = (currentPos - (double)lastState.q) / dt;
    // Simple low-pass filter to smooth velocity estimate
    currentVel = 0.3 * currentVel + 0.7 * (double)lastState.dq;
  } else {
    currentVel = (double)lastState.dq;  // Use previous velocity if dt is invalid
  }
  
  // Direct PD + feedforward control (bypass binary library computeTorque)
  // effort = Kp * (desired_pos - current_pos) + Kd * (desired_vel - current_vel) + feedforward_torque
  double pos_error = servoCmd.pos - currentPos;
  double vel_error = servoCmd.vel - currentVel;
  
  calcTorque = servoCmd.posStiffness * pos_error + 
               servoCmd.velStiffness * vel_error + 
               servoCmd.torque;
  
  // Apply effort limits
  effortLimits(calcTorque);

  // Safety check: if calcTorque is NaN or infinite, set to zero
  if (!std::isfinite(calcTorque)) {
    RCLCPP_WARN_THROTTLE(get_node()->get_logger(), *(get_node()->get_clock()), 1000,
                         "Invalid torque value (NaN or Inf) calculated for joint %s, setting to 0.0", 
                         joint_name_.c_str());
    calcTorque = 0.0;
  }
...
void UnitreeLeggedController::positionLimits(double &position){
  // Try to use URDF limits if available
  if (joint_ && joint_->limits) {
    clamp(position, joint_->limits->lower, joint_->limits->upper);
  } else {
    // Fall back to hardcoded limits based on joint name
    double lower, upper;
    if (joint_name_.find("hip") != std::string::npos) {
      // hip
...
void UnitreeLeggedController::velocityLimits(double &velocity){
  ...
  if (joint_ && joint_->limits) {
    ...
    // Fall back to hardcoded limits based on joint name
    double limit;
    if (joint_name_.find("hip") != std::string::npos || joint_name_.find("thigh") != std::string::npos) {
      // hip or thigh
      limit = 30.1;
    } else if (joint_name_.find("calf") != std::string::npos) {
      // calf
